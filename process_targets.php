...
//echo $currentdate;
        ...
        if ($type == '--Select--') {
            ...
            echo '<meta http-equiv="refresh" content="2;url=/blocks/group_targets/view.php">';
            exit;
        }
...
//set the things
        foreach ($checkboxes as $box) {
            ...
            if ($type == 'Target') {
                ...
                $query = "INSERT INTO progression_targets (studentid, target, setby, date) VALUES ('$box','" . $target . "','" . $userName . "','" . $date2 . "')";
                echo 'prog target' . $query;
                mysql_query($query);
                echo 'target set';
            } elseif ($type == 'Medals') {
...
//                    Update the new row
       ...
       echo '<meta http-equiv="refresh" content="0;url=/blocks/group_targets/view.php">';


        break;

    //submit a txt case switch
    case 'send txt':
...
//echo $idTxt;
        ...
        // Check the user has selected a message
        if ($idTxt == '--Select Message--') {
            echo '<h1>You must select a message - sending you back</h1>';
            echo '<meta http-equiv="refresh" content="2;url=/blocks/group_targets/view.php">';
        }

        // functions to modify and send out selected txts to students
// Select the message
        $query = 'SELECT * FROM txts WHERE idtxts="' . $idTxt . '"';
//echo $query;
        $result = mysql_query($query);
        echo '<table style="margin-left: auto; margin-right: auto;"><tr><td style="text-align: center;">';
        echo '<form name="process" action="process_txt.php" method="POST">';

        while ($row = mysql_fetch_assoc($result)) {
            echo '<h3>Make any changes needs to the text below or enter your message</h3>';
            echo '<p> You will need to change any test within and including {}</p>';
            echo '<TEXTAREA NAME="comments" COLS=40 ROWS=6>' . nl2br($row['message']) . '</textarea>';
        }
        foreach ($checkboxes as $box) {
            echo '<input type="hidden" name="checkbox[]" value="' . $box . '" />';
        }
        echo '<input type="hidden" name="url" value=/blocks/group_targets/view.php">';
        echo '</p>';
